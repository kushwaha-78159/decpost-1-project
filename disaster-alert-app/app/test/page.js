  ...
  const sendAlertToBackend = async (risk) => {
    try {
      await fetch("/api/send-alert", {
        ...
        body: JSON.stringify({ risk, location, weather: weatherData }),
      });
    } catch (error) {
    ...
    try {
      ...
      if (!apiKey || apiKey === "YOUR_API_KEY_HERE") {
        const demoWeather = {
          ...
          condition: ["clear sky", "few clouds", "scattered clouds"][Math.floor(Math.random() * 3)],
          timestamp: Date.now(),
        };
        setWeatherData(demoWeather);
        localStorage.setItem("lastWeatherData", JSON.stringify(demoWeather));
        toast.info("Using demo weather data. Add API key for real data!");
        return;
      }
